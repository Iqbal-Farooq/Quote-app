    ...
    >
      <div className="  flex items-center justify-between mb-2">
       <div className="flex gap-3 ">
        ...
        <div title="DELETE" className="flex justify-center align-center  bg-slate-300 hover:bg-[#cacecf] rounded-sm">
        <button className="text-xl  -mt-1.5  text-red-600 font-bold w-5 inline-flex items-center justify-center text-center" 
        onClick={()=>dispatch(removeItem(id))}> x</button>
        </div>
      </div>
      <div className="text-sm italic font-normal mb-3">{`" ${quote} "`}</div>
      <div className="flex gap-2">
        <button
          className="flex px-2 rounded-sm  bg-rgb(0, 114, 236) bg-[#005eec] active:bg-[#0072ec] text-center text-white"
          onClick={() => dispatch(addlikes(id))}
        >
          Likes {likes}
        </button>
        <button
          className="flex px-2 rounded-sm bg-[#3b4752] text-white active:bg-[#596978] text-center"
